...
def payload(processor):
    ...
    return {'rows': records(table), 'has_divisions': bool(divisions),
            'description': 'Standings through the report week, ordered by win percentage (ties count as half a win), then points for. Division rank is position within the division; overall rank is position across the league. Equal win percentage and points for share a rank. Division records count only games against teams in the same division. Points are calculated from actual starters; unavailable results remain blank. This ordering does not apply every Yahoo tiebreaker.'}
